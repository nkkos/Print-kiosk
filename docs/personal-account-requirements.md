...
- **The web portal** (`portal/`) — a separate product, accessed from the user's own device (phone, computer) via a normal browser, outside the kiosk application — see "Account lifecycle (portal)" below for what's actually implemented. Real backend now covers account lifecycle, folder/file management, and paid-in-advance orders (see `docs/screens/portal-personal-account-spec.md` for the visual redesign of these screens, in progress). Still out of scope: invoices, promo codes, and payment methods beyond the simulated "pay now" button.
...
**Implemented, real backend:** `LoginPanel` authenticates against `POST /api/accounts/login` (`server/routes.ts`, `server/accountStore.ts`), a real `accounts` table (`server/db/schema.ts`) with bcrypt-hashed passwords — no more hardcoded mock credential. Its "forgot password" mode is real too now (`POST /api/accounts/request-password-reset`), sending an actual email. **My files and My orders are real** — folders/files created on the portal (`server/accountFileStore.ts`) and orders paid in advance there (`server/accountOrderStore.ts`, simulated payment) are what the kiosk actually reads and lets the user select/print (`server/routes.ts`'s `sourceFileOrigin: 'account'` resolution path). **Registration, email verification, password reset completion, and change-password all live on the portal** (`portal/`), not the kiosk — see "Account lifecycle (portal)" below.

- **Baseline: email/password**, entered via the kiosk's on-screen keyboard (a physical keyboard is also expected to be installed at the kiosk). This must exist regardless of any other login method, since it's the only method that works for a first-time user with no prior session on any device. Email is the sole account identifier — there is no separate username, to avoid the confusion of two different handles for logging in vs. receiving account emails (verification, password reset).
...
This supersedes the "always created already-paid" behavior described under "Account lifecycle (portal)" below — not yet implemented. Needs, when built:

- `POST /api/accounts/orders` split into a create step (state 1, no payment yet) and a separate pay step (transition 1→2, same simulated "Pay now" as today).
- `printTasks` linked back to the `printOrders` row it printed (a new FK) so a successful print can drive transition 2→3. Must apply on **both** the real print-success path and the kiosk's manual `simulate` outcome (`docs/domain/kiosk-session.md` already documents `simulate` as standing in for a real terminal outcome for testing without a printer — it has to drive this transition too, not just the real path).
- **The kiosk's My orders stays scoped to state 2 only** ("paid, awaiting print" — see "Two separate surfaces" above: it was already confirmed to exclude the portal's full order history, not just fulfillment status). **The portal's My orders shows all three states** — it's the one surface with both a "pay" action (state 1→2) and a history view (state 3). Kiosk-facing reads (`GET /api/accounts/:accountId/orders`, the paid-orders-awaiting-print login prompt) filter to `status: 'paid'` specifically, unchanged in spirit from today.
...
- **`register.html`** — email, password → `POST /api/accounts/register`. Sends a verification email (`server/emailSender.ts`, via Resend) but does **not** block login on verification — an unverified account can still log in, since a frustrated user stuck at a public kiosk terminal over an unread email would be worse than the alternative.
...
Out of scope for this document: order history beyond "paid, awaiting print" (see "Open items" below — no fulfillment status exists yet to distinguish awaiting-print from already-printed), invoices, promo codes, and payment methods beyond the simulated "pay now" button, the full pricing/discount engine (promo codes, promotions, volume discounts — see `docs/domain/kiosk-session.md`, Open items, for early directional decisions only), corporate/B2B account features (shared team accounts, invoicing terms — explicitly deferred, not part of this method's current scope), and the six-method selection screen itself (see `docs/upload-method-requirements.md`).
...
- **Account file format/retention/storage-quota — confirmed as env-var-configured, exact values still open (2026-08-12):** Personal Account files need their own, narrower format list, a retention/TTL sweep, and a per-account storage quota — all different from today's real behavior (`server/accountFileStore.ts`/`server/fileValidation.ts`: accepts every QR/Email format, permanent storage, no quota). Confirmed these must be backend config values (env vars), not hardcoded, since the product owner expects to change them often without a code change. The wireframe's numbers (pdf+jpg, 30 days, 100 MB) are explicitly test values, not final — env var names and defaults still need deciding at implementation time. See `docs/screens/portal-personal-account-spec.md`.
