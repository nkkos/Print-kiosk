...
- **Dev-only fail-open, explicitly not a production decision:** if the scan engine itself is unreachable (e.g. a developer forgot to start `clamd`), the current implementation logs the failure server-side and lets the file through as `'ready'` rather than blocking every QR upload. A production system should fail closed instead — this is a deliberate dev-environment convenience, not a confirmed production behavior, and is called out here so it isn't mistaken for one.
