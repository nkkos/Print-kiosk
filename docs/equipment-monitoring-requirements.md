...
- **Real delivery confirmed end to end (2026-08-21), not just the dev fallback**: a real bot ("Print Kiosk Alerts", created via @BotFather) and a real personal chat id sent a genuine `critical` incident's message, received in Telegram — `notifiedAt` populated confirms the Telegram API call itself succeeded, not just that the code path ran. For a private 1:1 chat, `chat_id` is the recipient's own numeric Telegram user id (obtainable via `getUpdates` after messaging the bot once, or any "get my ID" utility bot) — the same id works for any bot messaging that person privately, not just this one.
- **Also confirmed on the real Railway deployment (2026-08-25)**, not just local dev: `TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` set on the `backend` service, a `critical` test incident triggered via Railway's Console (`reportIncident(...)` run directly against production), and the message arrived in Telegram. One real gotcha hit along the way: `notifyIfNeeded` is fire-and-forget (`void notifyIfNeeded(row)` inside `reportIncident`) — a one-off script that calls `reportIncident` and then `process.exit()` immediately can kill the process before that background send completes, silently dropping the notification with no error anywhere. Not a bug in the notification code itself — the long-running server process never does this — but worth remembering for any future one-off verification script.
