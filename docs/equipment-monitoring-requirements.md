...
- `queue-stuck` → restart the spooler service and resubmit the job once; if it still fails, stop and raise `critical` — no further retries (avoids wasting paper/ink on a doomed job).
...
- `submit-timeout` → no automatic retry in this pass (a hung printer usually needs a physical check, not a resubmission) — surfaces as `critical` and unblocks the queue for the next job; whether a bounded auto-retry ever makes sense here is an open question, not decided.
