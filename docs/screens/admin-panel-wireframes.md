...
- **Two roles**: a regular on-duty **operator** (sees everything, cannot trigger a remote fix) and a **senior** role (can). **Implemented:** role comes from logging in via a dedicated `staffAccounts`/`staffSessions` schema and `server/staffAccountStore.ts`/`server/adminRoutes.ts` — the same bcrypt+session-token mechanism `server/accountStore.ts` already uses, but a genuinely separate table, not a role column bolted onto the public, self-registering `accounts` table (revised from this document's original assumption — see the note further down).
...
**Resolved since this document was first written** (kept here for the record, not as open items anymore): login/role-assignment is **implemented** — reusing `accounts`'s bcrypt+session-token _mechanism_, but in a deliberately separate `staffAccounts`/`staffSessions` schema and `server/staffAccountStore.ts`, not a role column bolted onto the public, self-registering `accounts` table (revised from this document's original assumption — mixing self-service customer registration and staff privilege in one table was a real risk once actually building it, not just a hypothetical). No public registration route exists; provisioning is out-of-band (`server/scripts/seedStaffAccount.ts`). The panel gets its own plain/technical visual style, not the kiosk's design system (see the intro above); "restart backend process" is a real, confirmed action (`docs/equipment-monitoring-requirements.md`, Section E — implemented via the process calling `process.exit(1)` and relying on Railway's restart-on-crash policy).
