...
A React + TypeScript + Vite client application for a self-service printing kiosk (unattended document printing). Currently a clickable prototype: the full user journey works end-to-end against mock data (no automated tests — still explicitly out of scope per `docs/product-overview.md`). Real backend (`server/`) exceptions so far: QR and Email upload — see "Backend" below — the full account lifecycle (real accounts/bcrypt-hashed passwords, login, registration, email verification, password reset, change-password — kiosk login/forgot-password plus a separate minimal portal, `portal/`, for the rest; see "Backend" and `docs/personal-account-requirements.md`) — and real local-printer job submission on Print Status (`server/printerAdapter.ts`, dev-only, an early/partial start on hardware integration; see "Backend"). My files/My orders within Personal Account, Cart, and Payment remain mocked in the frontend; Print Status's underlying Cart/Print Order/pricing data is still mocked too — only the printer job-submission step itself is real.
...
- **Real database**: Postgres via Drizzle ORM (`server/db/schema.ts`, `server/db/client.ts`, migrations in `server/db/migrations/`) — see `README.md`, "Database," for local Docker setup. Migrations run automatically on boot (`server/index.ts`). `server/uploadStore.ts`/`server/emailStore.ts` are DB-backed (QR/Email upload records survive restarts/redeploys now); `kiosk_sessions`/`print_orders`/`payment_orders` tables exist for later phases (real payments, real per-account files/orders) but aren't wired into the frontend yet — Cart/Payment and My files/My orders remain mocked until those phases land. Uploaded files (QR and Email attachments alike) still land on disk under `server/uploads/<sessionId-or-email-prefix>/` (gitignored) — a Railway volume is needed there for the files themselves to survive a redeploy (the DB records survive regardless).
...
- **Real local-printer job submission** (`server/printerAdapter.ts`, via the `pdf-to-printer` npm package): `POST /api/print-tasks` submits one Cart item's real uploaded file (QR/Email only — see `docs/domain/kiosk-session.md`) to the machine's default (or a named) Windows printer, per-item `paperSize`/duplex-simplex/monochrome/`copies` forwarded from the real `PrintOrder`, tracked in the `print_tasks` table (`server/printTaskStore.ts`, not FK'd to `kiosk_sessions` since that table isn't written to yet). Formats `pdf-to-printer` can't handle directly (`.doc`/`.docx`/`.heic`) go through `server/documentConverter.ts` first (LibreOffice headless via `libreoffice-convert`, needs a one-time local LibreOffice install — see `README.md`; `.heic`→JPEG via `heic-convert`, no install needed) — any unresolvable file, unready scan, or conversion failure falls back to a placeholder PDF (`server/assets/print-test-page.pdf`). Only submission itself is real — a plain OS print API has no reliable in-progress signal (jam, out of paper/ink), so `POST /api/print-tasks/:id/simulate` drives those terminal outcomes manually from `PrintStatusScreen.tsx`, updating the same record a real failure would. Dev-only (`npm run dev:server`) — never deployed to Railway, since the physical printer lives on the kiosk PC, not in the cloud.
