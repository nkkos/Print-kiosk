...
A React + TypeScript + Vite client application for a self-service printing kiosk (unattended document printing). Currently a clickable prototype: the full user journey works end-to-end against mock data (no hardware integrations, no automated tests — still explicitly out of scope per `docs/product-overview.md`). Real backend (`server/`) exceptions so far: QR and Email upload — see "Backend" below — and Personal Account login (real accounts/bcrypt-hashed passwords, `POST /api/accounts/register`/`login`). My files/My orders within Personal Account, Cart, Payment, and Print remain mocked in the frontend.
...
- **Real accounts**: `accounts` table + `server/accountStore.ts`, used by `POST /api/accounts/register`/`login` (`server/routes.ts`) — bcrypt-hashed passwords (`bcryptjs`), `src/components/LoginPanel/LoginPanel.tsx` calls the real login endpoint via `src/services/accountApi.ts`. No session token yet — nothing beyond login itself needs to be an authenticated request so far (see `docs/personal-account-requirements.md`, "Kiosk-side login").
