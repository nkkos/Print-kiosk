...
| `NODE_ENV`                  | backend         | unset                                                     | Set to `production` on Railway — gates antivirus-scan-unavailable behavior (dev fails open; production deletes the file and shows a distinct message, see `docs/domain/kiosk-session.md`, "File scanning status"). Not set automatically — must be added explicitly. |
| `PRINTER_QUEUE_BIN_1`…`_4`  | backend         | unset                                                     | Kiosk PC only. Windows print queue per MX-4000 mailbox bin (each queue's Printing Defaults pinned to "MX bin N") — the reserved pickup bin picks the queue. Unset → Windows default printer. See `server/printerAdapter.ts`.                                         |
| `PRINTER_TRAY_A4` / `_A5`   | backend         | unset                                                     | Kiosk PC only. Input tray per paper size (Brother codes: `1`=Tray1, `2`=Tray2/LT-330CL, `258`=MP tray). Unset → driver auto-selects by size.                                                                                                                         |
| `PRINT_EXECUTION`           | backend         | `direct`                                                  | `direct`: this backend prints to its own machine's printer (local dev). `agent`: set on Railway — the backend only reserves the bin; the pavilion print agent claims and prints the job (see "Print agent" below).                                                   |
| `PRINT_AGENT_TOKEN`         | backend + agent | unset                                                     | Shared secret the print agent sends as `Authorization: Bearer …`. Unset → every `/api/agent/*` route answers 503. Use a long random value, identical on both sides.                                                                                                  |
| `AGENT_CLOUD_URL`           | agent           | unset                                                     | Backend base URL the agent polls, e.g. the Railway public domain with `https://`.                                                                                                                                                                                    |
...
Printer setup on the mini-PC: `agent\windows\setup-printer.ps1 -PrinterIp <ip> -DriverInf <path to the Brother driver's gdi\BROCH19A.INF>` (elevated) installs the Brother driver, creates the TCP/IP port and the `HL9430-Bin1..4` / `HL9430-Staff` queues, pins each queue's Printing Defaults to its mailbox bin (via the driver's PrintTicket `JobOutputBin` option) and prints the `.env` lines for the agent. Safe to re-run; `-CaptureDriverOutput` also captures the driver's PJL per bin into `logs\driver-capture\` without using paper.

On the mini-PC: clone the repo, `npm ci`, create `.env` with `AGENT_CLOUD_URL`, `PRINT_AGENT_TOKEN` and the `PRINTER_*` variables, then try it with `npm run agent`. To keep it running for real, run `agent\windows\install-agent-task.ps1` once in an elevated PowerShell (`powershell -ExecutionPolicy Bypass -File agent\windows\install-agent-task.ps1`): it registers a `PrintKioskAgent` task that starts at boot as SYSTEM (no login needed) and runs `agent\windows\run-agent.ps1`, which restarts the agent 10 s after any exit and logs to `logs\agent.log`. `uninstall-agent-task.ps1` removes it.

Kiosk stands: start each stand's browser on the kiosk URL with `?stand=A` (or `B`) — the stand remembers it (`src/utils/standId.ts`), sends it with every print job, and staff see it in the admin Print Queue. The cart refuses payment while `GET /api/printer-status` says the printer is unavailable.
...
3. **`backend`** — connect this GitHub repo. Railway builds it from the root `Dockerfile` (Node + LibreOffice with Word-compatible fonts, so `.doc`/`.docx` conversion works in the cloud); the image's command is already `npm start`, the production entrypoint. Reference the Postgres service's `DATABASE_URL` into this service's variables (Railway's variable-reference UI, e.g. `${{Postgres.DATABASE_URL}}`) — migrations run automatically on boot, nothing else to do. Set `CLAMD_HOST=clamav.railway.internal` and `CLAMD_PORT=3310`, and `NODE_ENV=production` (gates antivirus-fail-closed behavior — see "Backend environment variables" above). Generate a public domain for it (Settings → Networking) — this is what `RAILWAY_PUBLIC_DOMAIN` picks up automatically and what QR's `GET /api/config` and the Cloudflare Worker's `BACKEND_URL` both point at. A persistent volume for `server/uploads/` is recommended (without one, uploaded files disappear on every redeploy — the database records survive, but the file bytes are on this service's local disk).
